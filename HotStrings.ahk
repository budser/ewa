...
::-ws:: ; Highlight an address and type "-ws" to open the relevant walk score
::-walkscore:: ; Highlight an address and type "-walkscore" to open the relevant walk score
SleepTime := 250 ; Set the time to sleep between steps to .25 seconds
ComaTime := 2000 ; Long sleep for 2 seconds
ClipBoard := ""

Sleep, %SleepTime%
SendInput ^c ; Copy highlighted address
Sleep, %SleepTime%
if(ClipBoard <> "") ; Clipboard not empty because user remembered to highlight address before hostring called
{
	Run https://www.walkscore.com/
	Sleep, %ComaTime%
	SendInput ^f
	Sleep, %SleepTime%
	SendInput Live Where You Love
	Sleep, %SleepTime%
	SendInput {Escape}{Tab} 
	Sleep, %SleepTime%
	StringReplace, Clipboard, Clipboard, `r`n, %A_SPACE%, All ; Replace carriage returns/enters with spaces  
	SendInput %Clipboard%
	Sleep, %SleepTime%
	SendInput {Enter}
	Return 
